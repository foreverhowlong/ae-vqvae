"""Visualize AE reconstructions and 2D latent space."""
...
def load_model(pth_path=model_path):
    device = get_device()
    model = load_state_dict(AE().to(device), pth_path, device)
    return model, device
...
def show_reconstruction(pth_path=model_path, num_pairs=8):
    ...
    show_reconstruction_grid(
        ...
        labels=labels,
        title="AE Reconstruction Comparison",
    )
...
def plot_latent_space(pth_path=model_path):
    ...
    latents, labels = collect_latents(
        model, device, encode_fn=lambda m, x: m.encoder(x),
    )
    plot_latent_scatter(latents, labels, "AE Latent Space (2D) - MNIST Test Set")
